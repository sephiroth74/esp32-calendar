...
ErrorInfo ErrorManager::currentError =
    {ErrorCode::SUCCESS, ErrorLevel::INFO, ErrorIcon::NONE, "", "", 0, true, 0, 0};
ErrorInfo ErrorManager::lastError =
    {ErrorCode::SUCCESS, ErrorLevel::INFO, ErrorIcon::NONE, "", "", 0, true, 0, 0};
...
bool ErrorManager::hasActiveError() { return hasError; }

ErrorInfo ErrorManager::getCurrentError() { return currentError; }

ErrorInfo ErrorManager::getLastError() { return lastError; }

bool ErrorManager::isCritical() { return hasError && (currentError.level == ErrorLevel::CRITICAL); }

bool ErrorManager::shouldRetry() {
    return hasError && currentError.recoverable &&
...
ErrorLevel ErrorManager::getErrorLevel(ErrorCode code) {
    ...
    case (int)ErrorCode::OTA_UPDATE_AVAILABLE:     return ErrorLevel::INFO;

    // Warning level
    case (int)ErrorCode::WIFI_WEAK_SIGNAL:
    case (int)ErrorCode::BATTERY_LOW:
    case (int)ErrorCode::MEMORY_LOW:
    case (int)ErrorCode::CALENDAR_TOO_MANY_EVENTS: return ErrorLevel::WARNING;
    ...
    case (int)ErrorCode::CONFIG_CALENDAR_NOT_SET:  return ErrorLevel::CRITICAL;
...
ErrorIcon ErrorManager::getErrorIcon(ErrorCode code) {
    if ((int)code >= 100 && (int)code < 200)
        return ErrorIcon::WIFI;
    if ((int)code >= 200 && (int)code < 300)
        return ErrorIcon::CALENDAR;
    if ((int)code >= 300 && (int)code < 400)
        return ErrorIcon::CLOCK;
    if ((int)code >= 400 && (int)code < 500)
        return ErrorIcon::GENERAL;
    if ((int)code >= 500 && (int)code < 600)
        return ErrorIcon::BATTERY;
    if ((int)code >= 600 && (int)code < 700)
        return ErrorIcon::MEMORY;
    if ((int)code >= 700 && (int)code < 800)
        return ErrorIcon::NETWORK;
    if ((int)code >= 800 && (int)code < 900)
        return ErrorIcon::SETTINGS;
    if ((int)code >= 900 && (int)code < 1000)
        return ErrorIcon::UPDATE;
    return ErrorIcon::GENERAL;
}
...
bool ErrorManager::isRecoverable(ErrorCode code) {
    ...
    case ErrorCode::CONFIG_CALENDAR_NOT_SET:  return false;
    default:                                  return true;
    }
}
...
int ErrorManager::getMaxRetries(ErrorCode code) {
    ...
    case ErrorCode::NTP_SYNC_FAILED:          return 5;

    case ErrorCode::CALENDAR_FETCH_FAILED:
    case ErrorCode::CALENDAR_TIMEOUT:
    case ErrorCode::NETWORK_TIMEOUT:
    case ErrorCode::HTTP_ERROR:               return 3;
...
bool ErrorManager::isErrorActive(ErrorCode code) { return hasError && (currentError.code == code); }