...
bool LittleFSConfig::loadConfiguration() {
    ...
    if (doc.containsKey("wifi")) {
        ...
        Serial.println("  Password extracted: " +
                       String(config.wifi_password.isEmpty() ? "(empty)" : "(set)"));
    } else {
    ...
    if (doc.containsKey("calendars")) {
        ...
        if (doc["calendars"].is<JsonArray>()) {
            ...
            for (JsonVariant cal : calendars) {
                ...
                Serial.println("    URL: " + (calConfig.url.isEmpty()
                                                  ? "(empty)"
                                                  : calConfig.url.substring(0, 50) + "..."));
                ...
                Serial.println("    Holiday calendar: " +
                               String(calConfig.holiday_calendar ? "yes" : "no"));
...
void LittleFSConfig::clearCalendars() { config.calendars.clear(); }
...
void LittleFSConfig::printConfiguration() {
    ...
    Serial.println("  Password: " +
                   String(config.wifi_password.isEmpty() ? "[not set]" : "********"));
