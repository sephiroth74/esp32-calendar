...
// Button electrical level when pressed.
// LOW  = active-low button (external pull-up, button to GND)
// HIGH = active-high button (external pull-down, button to 3.3V)
#define BUTTON_ACTIVE_LEVEL HIGH

// Safety check: button and battery monitor cannot share the same GPIO
#if BUTTON_PIN == BATTERY_PIN
#error "BUTTON_PIN and BATTERY_PIN must be different GPIOs"
#endif
...
#define RGB_LED_BRIGHTNESS 25 // About 10% brightness for power efficiency
